...
<?= $this->section('content') ?>
    ...
    <?=form_textarea('description', is_null(old('description')) ? $content['description'] : old('description'), ['rows' => 4])?>
    <?=validation_show_error('description')?>

    <?=form_label('Content Body:', 'content_body')?>
    <?=form_textarea('content_body', is_null(old('content_body')) ? $content['content_body'] : old('content_body'), ['rows' => 12])?>
    <?=validation_show_error('content_body')?>

    <?=form_label('Content Type:', 'content_type_id')?>
    <?=form_dropdown('content_type_id', $content_types, is_null(old('content_type_id')) ? $content['content_type_id'] : old('content_type_id'))?>
    ...
    <?=form_checkbox('published', true, is_null(old('published')) ? $content['published'] : old('published'))?>
